import { SetStateAction, Dispatch, useEffect, useState, useOptimistic } from 'react';
      ...
      {todos.length < 1 || isLoading ? (
        ...
        <ul className="space-y-2">
          {optimisticTodos.map((todo, index) => (
            <ListItem optimistic={todo.optimistic} key={index} id={todo.id} text={todo.text} />
          ))}
        </ul>
      )}
    </>
...
const ListItem = ({ id, text, optimistic }: { id: any; text: string; optimistic?: boolean }) => (
  <li key={id} className={`flex items-center p-3 border rounded-md transition-all`}>
    ...
    {optimistic && <p className="text-black">...Loading</p>}
  </li>
);
