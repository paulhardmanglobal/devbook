...
export default function OptimisticUpdatesDemo() {
  const [initialTodos, setIniitalTodos] = useState<Todo[]>([]);
  const fetchTodos = async () => {
    const result = await fetch('/api/todos');
    const { todos } = await result.json();
    setIniitalTodos(todos);
  };
  ...
  const handleSubmitForm = async (prev: Todo[], formData: FormData) => {
    const todo = {
  ...
  const [todos, formAction, isPending] = useActionState<Todo[], FormData>(
    handleSubmitForm,
    initialTodos
  );
